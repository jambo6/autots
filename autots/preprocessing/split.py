...
def train_val_test_split(
    ...
    if stratify_idx is not None:
        stratification_labels = tensors[stratify_idx] if tensors[stratify_idx].dim() <= 2 else None
...
def tensor_train_test_split(tensors, test_frac, stratify=None, shuffle=True, random_state=None):
    ...
    split_tensors = train_test_split(
        *tensors, stratify=stratify, shuffle=shuffle, test_size=test_frac, random_state=random_state
    )
...
class TrainValTestSplit:
    ...
    def __init__(self, val_frac=0.15, test_frac=0.15, stratify=False, shuffle=True, random_state=None):
        """
        Args:
    ...
    def _assertions(self):
        ...
        if self.val_frac is not None:
            ...
            assert self.val_frac + self.test_frac < 1, "val_frac and test_frac must sum less than 1"
    ...
    def split(self, num_samples, stratification_tensor=None):
        ...
        """
        assert isinstance(num_samples, int), "num_samples of the data must be inserted as an integer"
        ...
        tensors, stratify_index, = (
            ([all_indices, stratification_tensor], 1) if stratification_tensor is not None else ([all_indices], None)
        )
